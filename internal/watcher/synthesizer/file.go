...
func synthesizeFileAuths(ctx *SynthesisContext, fullPath string, data []byte) []*coreauth.Auth {
	...
	// Use relative IDs only when scanning the primary auth-dir; extra pool dirs keep full-path IDs
	// so entries from different directories remain unique without editing auth JSON files.
	id := fullPath
	...
	if group := resolveAuthGroup(ctx.Config, fullPath, metadata); group != "" {
